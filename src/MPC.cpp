 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      fg[0] += 150 * CppAD::pow(vars[a_start + i], 2);
    }
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> func = coeffs[0] + coeffs[1] * x0 + coeffs[2] * CppAD::pow(x0, 2) + coeffs[3] * CppAD::pow(x0, 3) + coeffs[4] * CppAD::pow(x0, 4) + coeffs[5] * CppAD::pow(x0, 5); 
      AD<double> diff = coeffs[1] + 2 * coeffs[2] * x0 + 3 * coeffs[3] * CppAD::pow(x0, 2) + 4 * coeffs[4] * CppAD::pow(x0, 3) + 5 * coeffs[5] * CppAD::pow(x0, 4);
      AD<double> psi_des = CppAD::atan(diff);
